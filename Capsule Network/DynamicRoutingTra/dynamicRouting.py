...
class ConvLayer(nn.Module):
    def __init__(self, args):
        ...
        self.in_channels = args.inputConvChannels
...
class TransformerBlock(nn.Module):
    def __init__(self, d_model=128, nhead=8, num_layers=2, dim_feedforward=256, dropout=0.1):
        ...
        self.flatten = nn.Flatten(2)  # Flatten spatial dimensions (20, 20) to sequence length 400
        encoder_layer = nn.TransformerEncoderLayer(d_model=d_model, nhead=nhead, dim_feedforward=dim_feedforward,
                                                   dropout=dropout)
        self.transformer_encoder = nn.TransformerEncoder(encoder_layer, num_layers=num_layers)

    def forward(self, x):
        batch_size, channels, height, width = x.size()  # x is (batch_size, 128, 20, 20)
        x = self.flatten(x)  # Output: (batch_size, 128, 400)
        x = x.permute(2, 0, 1)  # Transform to (sequence_length, batch_size, d_model)
        x = self.transformer_encoder(x)  # Output: (sequence_length, batch_size, d_model)
        x = x.permute(1, 2, 0)  # Transform back to (batch_size, d_model, sequence_length)
        x = x.view(batch_size, channels, height, width)  # Reshape back to (batch_size, 128, 20, 20)
        return x
...
class Decoder(nn.Module):
    def __init__(self, args):
        ...
        self.inputChannels= args.inputConvChannels
...
class CapsuleNetwork(nn.Module):
    ...
    def forward(self, images):
        ...
        x = self.transformer_block(x)  # Output: (batch_size, 128, 20, 20)
        ...
        #     reconstructions, y, logits = self.decoder(caps_output)
        # Ensure caps_output is 3D: [batch_size, num_classes, capsule_dim]
        if caps_output.dim() == 2:
            # [num_classes, capsule_dim] → [1, num_classes, capsule_dim]
            caps_output = caps_output.unsqueeze(0)
            reconstructions, y, logits = self.decoder(caps_output)
        elif caps_output.dim() == 3 and caps_output.shape[0] != images.shape[0]:
            # [num_classes, batch_size, capsule_dim] → [batch_size, num_classes, capsule_dim]
            caps_output = caps_output.permute(1, 0, 2)
            reconstructions, y, logits = self.decoder(caps_output)
