...
class CapsNet(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.classesNum = int(args.classesNum)
        K=3
        P=4
        FF=8
        G=8
        self.conv1 = nn.Conv2d(in_channels=args.inputConvChannels, out_channels=args.CapsA,
        ...
        self.conv_caps1_1 = ConvCaps(args.CapsB, args.CapsC, K, P, stride=1, iters=args.iters)
        ...
        self.decoder = nn.Sequential(
            ...
            nn.Linear(512, 1024),
            nn.ReLU(),
            nn.Linear(1024, args.inputConvChannels * args.inputSize * args.inputSize),
            nn.Sigmoid()
        )#784 , 2304 , 4096, 4096

        
    def forward(self, x, y, recon=False, probe=False):
        x = self.conv1(x)
        x = self.bn1(x)
        #x = self.convd(x)
        x = self.relu1(x)
        ...
        if recon:
            ...
            d_in = torch.zeros(target_pose.shape[0], target_pose.shape[1] * self.classesNum, device=x.device)
